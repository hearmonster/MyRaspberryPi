...
#
#	COMMANDS: Requires the <Device AltID> (Topic) and the <Property Name> of each Capability
...
sDeviceCredentialsKeyFilename  = 'MyRaspberryPi-Device-credentials.key'		#parser['device']['credentialsKey']
sDeviceCredentialsCertFilename = 'MyRaspberryPi-Device-credentials.crt'		#parser['device']['credentialsCert']
	...
	while client.continueLoop:
		...
		while not exit.is_set():
			...
			try:
				...
				logging.info("result of publish for capability >>>DHT<<<  : " + str(sResult) + "\n")
				...
				sResult=client.publish(my_publish_topic, sPayload, qos=0)
				logging.info("result of publish for capability >>>Light<<<  : " + str(sResult) + "\n")
